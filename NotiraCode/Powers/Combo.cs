...
protected override IEnumerable<IHoverTip> ExtraHoverTips
    {
        get
        {
            yield return HoverTipFactory.Static(
                StaticHoverTip.Block
            );

            foreach (var play in CombatManager.Instance.History
                         .CardPlaysFinished
                         .TakeLast(2))
            {
                yield return new CardHoverTip(
                    play.CardPlay.Card.ToMutable()
                );
            }
        }
    }
